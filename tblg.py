...
def tblg_nm(n=9, m=8, d=1):
    ...
    Args:
        n (int, optional): sc 1
        m (int, optional): sc 2
        d (float, optional): interlayer distance
    Returns:
    ...
    lattice_lower = [
        3 * R(t / 2) @ i
        for i in np.array([[1.0, 0.0, 0], [0.5, np.sqrt(3.0) /
                                           2.0, 0], [0, 0, 10]])
    ]  # lower lattice
    lattice_upper = [
        3 * R(-t / 2) @ i
    ...
    lower = Structure(lattice=lattice_lower,
                      species=["C", "C"],
                      coords=[[1. / 3., 1. / 3., 0.5], [2. / 3., 2. / 3.,
                                                        0.5]])
    upper = Structure(lattice=lattice_upper,
                      species=["C", "C"],
                      coords=[[1. / 3., 1. / 3., 0.5], [2. / 3., 2. / 3.,
                                                        0.5]])
    ...
    sc[1][0] = -m
    sc[1][1] = n + m
    lower.make_supercell(
        sc)  # supercell transfornation matrix for lower system
    ...
    sc[1][0] = -n
    sc[1][1] = m + n
    upper.make_supercell(
        sc)  # supercell transfornation matrix for upper system
    ...
    for i in lower:
        pos.append(i.coords + [0, 0, -d])
    species = ["C"] * len(pos)
    tblg = Structure(lattice=upper.lattice,
                     coords=pos,
                     species=species,
